...
#Crear un diccionario con los datos ingresados
    paciente={
        ...
        "Estatura del Paciente": estatura_paciente.get()
    }
...
comboTipoSeguro=ttk.Combobox(frame_pacientes, values=["Público", "Privado"], textvariable=tipo_seguro)
...
comboCentroMedico=ttk.Combobox(frame_pacientes, values=["Hospital San Juan de Dios", "Clinica Norte", "Centro Sur"], textvariable=centro_medico)
comboCentroMedico.grid(row=7, column=1, sticky="w", padx=5, pady=5)
#Estatura de paciente
...
estatura_paciente=tk.StringVar()
estatura_paciente=tk.Entry(frame_pacientes)
estatura_paciente.grid(row=8, column=1, sticky="w", pady=5, padx=5)
